...
/**
 * Pega as contas da API, armazena, processa e renderiza a UI inicial.
 */
...
/**
 * Processa uma lista de contas, agrupando por mês e calculando totais.
 */
function processarErenderizar(listaDeContas) {
    dadosPorMes = listaDeContas.reduce((acc, conta) => {
        ...
        const mesKey = `${data.getFullYear()}-${String(data.getMonth() + 1).padStart(2, '0')}`; // Formato: YYYY-MM
...
/**
 * Cria e renderiza as abas de navegação dos meses.
 */
...
/**
 * Renderiza o resumo e a tabela para um mês específico.
 */
function renderizarDadosDoMes(mesKey) {
    ...
    dadosDoMes.contas.forEach(conta => {
        ...
        tr.innerHTML = `
            ...
            <td>${botaoToggle}<button class="btn btn-sm btn-danger btn-excluir" data-id="${conta.id}">❌ Excluir</button></td>
        `;
...
// Pesquisa (filtra os dados do lado do cliente)
...
formNovaConta.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        formNovaConta.reset();
        inicializar(); // Recarrega e reprocessa todos os dados
    } catch (error) {
...
tabelaContas.addEventListener('click', async (event) => {
    ...
    if (acaoRealizada) inicializar(); // Recarrega tudo se uma ação foi feita
});
